...
def main():
    ...
    hidetext = 30 * " "
    while True:
        # recupere la position de la voiture rouge
...
def makeLevelButtons():
    ...
    for j in range(LEVELHEIGHT):
        for i in range(LEVELWIDTH):
            ...
            pygame.draw.rect(windowSurface, BUTTONCOLOR, (
            left + tileborder, top + tileborder, TILESIZE - 2 * tileborder, TILESIZE - 2 * tileborder))
            number += 1
...
def drawBoard():
    ...
    borderwidth = 20
    tileborder = 4
    pygame.draw.rect(windowSurface, TILECOLOR,
                     (left - borderwidth, top - borderwidth, width + 2 * borderwidth, height + 2 * borderwidth))
    pygame.draw.rect(windowSurface, BOARDCOLOR,
                     (left - tileborder, top - tileborder, width + 2 * tileborder, height + 2 * tileborder))
    for j in range(BOARDHEIGHT):
        for i in range(BOARDWIDTH):
            pygame.draw.rect(windowSurface, TILECOLOR, (
            left + i * TILESIZE + tileborder, top + j * TILESIZE + tileborder, TILESIZE - 2 * tileborder,
            TILESIZE - 2 * tileborder))
    pygame.draw.rect(windowSurface, BOARDCOLOR, (
    left + BOARDWIDTH * TILESIZE, top + 2 * TILESIZE - tileborder, borderwidth, TILESIZE + 2 * tileborder))
...
class Car(object):
    ...
    def drawCar(self):
        ...
        carborder = 2
        pygame.draw.rect(windowSurface, self.GetCarColor(),
                         (left + carborder, top + carborder, width - 2 * carborder, height - 2 * carborder))
