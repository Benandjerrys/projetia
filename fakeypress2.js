...
function run() {
    ...
    if (checkCookie()) {
        if ($.cookie(cookieName)
            ...
            UI.ErrorMessage("A cause d'une mise a jour, les donnÃ©es ont Ã©tÃ©s perdues. Veuillez recommencer les assignations de touches.", 5000);
...
function addTable() {
    ...
    $('#divFAPress')
        .append($(
            "<table id='faKeySettings' class='vis' style='width:100%' cellspacing='0'><thead><tr><th colspan='3'><em>ParamÃªtres</em> - <a href'#' id='showSettings' onclick='return doSettings()'>CachÃ©</a></thead><tbody id='bodySettings'><tr><td colspan='1' align='center'><input type='checkbox' id='chbLoadPages' onclick='return chkBoxClick($(this).is(\":checked\"), " +
            ...
            "' onchange='onlyNum(this);' disabled><tr><td align='center'><b>CachÃ©</b><td><input type='checkbox' id='chbRemAxes' onclick='return chkBoxClick($(this).is(\":checked\"), " + pos.s
            .remaxes +
            ...
            pos.s.remred +
            ")'> <img src='https://media.innogamescdn.com/com_DS_FR/Scripts/Pillage/red.png' title='DÃ©fait' alt='' class='tooltip' /> DÃ©fait</tr><tr><td align='right' colspan='2'><input type='button' class='btn' id='btnSettingsReset' value='Reset' onclick='resetCookie(); UI.SuccessMessage(\"Reset effectuÃ©\",1000); run(); return false;'><input type='button' class='btn' id='btnSettingsApply' value='Appliquer' onclick='saveSettings(); run(); return false'><input type='button' class='btn' id='btnSettingsSave' value='Sauvegarder' onclick='saveSettings(); return false;'></tr></tbody></table>"
        ));
...
function saveSettings() {
    ...
    userset[pos.s.MaxNbAttacks] = $('#txtNbAttacks')
        .val();
    setCookie(cookieName, "{" + version + "}[" + userkeys.toString() + "][" + userset.toString() + "]", 180);
    UI.SuccessMessage("ParamÃ¨tres sauvÃ©s", 1000);
}
...
function openLoader() {
    ...
    setTimeout(function() {
        if (incrementalSwitchPage <= currentIncremental) {
            UI.ErrorMessage('Un problÃ¨me a Ã©tÃ© rencontrÃ©, la page va se recharger.');
            var currentURL = document.URL.split("village=") + "village=" + game_data.village.id + "&screen=am_farm";
            window.location.href = currentURL;
			console.log("La page a mis trop de temps Ã  se charger.")
        }
    }, 4000);
